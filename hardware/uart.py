  ...
  if miss > 5 and len(buffer)>0:
    if not len(buffer.hex()) == 80:
      ...
      print(colored(f"{len(buffer.hex())}: ","green"),
            ...
            f"PM1.0: {int.from_bytes(buffer[4:6],'big')}, {int.from_bytes(buffer[10:12],'big')}","|",
            f"PM2.5: {int.from_bytes(buffer[6:8],'big')}, {int.from_bytes(buffer[12:14],'big')}","|",
            f"PM10: {int.from_bytes(buffer[8:10],'big')}, {int.from_bytes(buffer[14:16],'big')}","|",
            ...
            f"Temp: {buffer[24:26].hex()}, {int.from_bytes(buffer[24:26],'big')}"," | ",
            f"Humi: {buffer[26:28].hex()}, {int.from_bytes(buffer[26:28],'big')}"," | ",
            f"Data: {buffer[28:30].hex()}, {int.from_bytes(buffer[28:30],'big')}"," | ",
            f"Data: {buffer[30:32].hex()}, {int.from_bytes(buffer[30:32],'big')}"," | ",
            f"Data: {buffer[32:34].hex()}, {int.from_bytes(buffer[32:34],'big')}"," | ",
